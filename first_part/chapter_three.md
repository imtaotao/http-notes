...
```html
  ...
  ------------------------------------------------------------------------------------------------------- 
```
①[十七章](../four_part/chapter_seventeen.md)<br>
...
+ 问题出在 HTTP/1.1。 HTTP/1.1 规范使用 303 状态码来实现同样的行为。为了避开这个问题，HTTP/1.1 规范指出，对于 HTTP/1.1 客户端，用 307 状态码取代 302 状态码来进行临时重定向。这样服务器就可以将 302 状态码保留起来，为 HTTP/1.0 客户端使用了。（个人理解 302 这个状态码在 1.0 的协议中有两种功能，分别对应这个 1.1 协议中的 303 和 307，1.1 协议应该是把这俩功能拆开了，正好是 1.1 要做的事情，设计中的结构性缺陷，明确语义）
...
## 首部
首部与方法配合工作，共同决定了客户端和服务器能做什么事情。附录 C 对所有首部进行了详细的总结。首部可以分为以下五个类型
