...
+ **媒体类型**<br>
HTTP 会给每种要通过 Web 传输的对象都打上名为 `MIME` 类型的数据格式标签（最初 MIME 是在电子邮箱系统中，HTTP 是后来借鉴了它）<br>
Web 服务器会为所有的 HTTP 对象数据附加一个 MIME 类型（附录 D 提供了一个完整的 MIME 类型列表）

+ **URI**<br>
服务器资源名被称为**统一资源标识符**（URI），在世界范围内唯一标识并定位信息资源。URI 有两种形式，**统一资源定位符**（URL）和**统一资源名**（URN）`
...
## 报文
报文分为**请求报文**和**响应报文**，没有其他类型的报文，[第三章](./chapter_three.md)会深入 http 报文。报文包含以下三个部分
